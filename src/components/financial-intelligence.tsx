...
export function FinancialPositionWorkspace() {
  ...
  return <section className="position-workspace">
    ...
    <section className="workspace-panel net-worth-history"><div className="section-header"><div><h2>Net-worth history</h2><p>Updates automatically when account balances, liabilities, or linked transactions change.</p></div></div><div className="position-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={history.map(item=>({...item,date:new Date(item.date).toLocaleDateString()}))}><CartesianGrid vertical={false} stroke="#dce2ea"/><XAxis dataKey="date" axisLine={false} tickLine={false}/><YAxis axisLine={false} tickLine={false}/><Tooltip/><Area dataKey="netWorth" stroke="#2f6fec" fill="#2f6fec" fillOpacity={.15}/></AreaChart></ResponsiveContainer></div></section>
...
function SafeChart({ spec }: { spec: AnalysisChartSpec }) {
  ...
  const axes = <><CartesianGrid vertical={false} stroke="#dce2ea" /><XAxis dataKey="x" axisLine={false} tickLine={false} tick={{ fontSize: 10 }} /><YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10 }} /><Tooltip formatter={tooltipFormatter} /></>;
